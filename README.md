...
This is the world's first visual web component, built with features and Declarative Custom Element cede scripts.  It is "code-free". In particular, it draws from the [el-maker](https://www.npmjs.com/package/el-maker) base class module package, only loading those features that are actually used, as needed.  It is entirely built with a static html file [root.html](/root.html) and a JSON file [el-maker.json](/el-maker.json).  Editing JSON by hand is error-prone and doesn't lend itself to Typescript checking.  So the JSON is built from [el-maker.mjs](/el-maker.mjs) using command
